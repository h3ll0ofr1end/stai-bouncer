...
type StaiCli struct {
	// StaiExecutable represents the stai executable e.g. /home/steffen/stai-blockchain/venv/bin/stai
	StaiExecutable string
}

// ListNodes will list all connected FullNodes.
// It basically acts as wrapper around "stai show -c"
func (c StaiCli) ListNodes() (nodes []FullNode, err error) {
	out, err := execCmd(c.StaiExecutable, "show", "-c")
	if err != nil {
		return nil, err
	}
...
// convertToNodes filters the stdout of "stai show -c" command for FullNodes and
...
// convertToNode will convert a single FULL_NODE line of the stdout of the "stai show -c" command.
...
func (c StaiCli) RemoveNode(nodeId string) error {
	if _, err := execCmd(c.StaiExecutable, "show", "-r", nodeId); err != nil {
		return err
	}
	return nil
}
