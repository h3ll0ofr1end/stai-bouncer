...
import (
	"fmt"
	"github.com/h3ll0ofr1end/stai-bouncer/bouncer"
	"os"
)
...
func run() error {
	...
	staiCli := bouncer.StaiCli{StaiExecutable: ctx.StaiExecutable}
	nodes, err := staiCli.ListNodes()
	if err != nil {
		return err
	}
	...
	for _, node := range filterSet {
		if err := staiCli.RemoveNode(node.NodeId); err != nil {
